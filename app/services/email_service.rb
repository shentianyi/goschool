...
class EmailService
  ...
  def self.send_schedule_email type, institution_id, user_ids=nil
    case type
      ...
      when ScheduleType::STUDENT
        puts 'student-----------'
        send_student_schedule_email type, institution_id, user_ids
      else
        nil
    end
  end
  ...
  def self.send_student_schedule_email type, institution_id, student_ids=nil
    if student_ids
      Student.where(id: student_ids).where(Student.arel_table[:email].not_eq(nil)).each do |student|
        css=CourseScheduleService.new({type: type, institution_id: institution_id, student_id: student.id})
        if schedules=css.generate_schedule
          puts student.email
          send_schedule schedules, student.email
        end
      end
    else
