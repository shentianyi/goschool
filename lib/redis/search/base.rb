    ...
    module ClassMethods
      ...
      def redis_search_index(options = {})
        ...
        class_eval %(
          ...
          def redis_search_index_delete(titles,updated=false)
            condi={}
           
            if updated
                #{condition_fields}.each do |c|
                  condi[c]=  instance_eval(c.to_s+'_was') if  instance_eval(c.to_s+'_changed?') 
               end 
             else
            #{condition_fields}.each do |c|
             condi[c]=  instance_eval(c.to_s)
            end 
            end
            
            titles.uniq.each do |title|
              Search::Index.remove(:id => self.id, :title => title, :type => self.class.to_s,:condition_fields=>condi)
            end
            true
          end
          ...
          after_update do
            if self.redis_search_index_need_reindex
              ...
              redis_search_index_delete(titles,true)
            end
            true
          end
