...
class StudentNotAttendedCoursesCode
  ...
  def query(query_obj,parameters)
    ...
    return query_obj.where('students.id not in (select student_id from student_courses,courses where courses.id=student_courses.id and courses.code = ?', parameters[0])
  end
